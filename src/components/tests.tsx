                ...
                <div className="modal fixed top-0 left-0 w-full h-full bg-gray-900 bg-opacity-50 flex justify-center items-center">
                    <div className="modal-content bg-gray-200 p-6 rounded-lg flex">
                        <div className="flex-1">
                            ...
                            <form
                                onSubmit={e => {
                                    ...
                                    const absoluteTestFileName = testFileName.startsWith(
                                        './'
                                    )
                                        ? `${rootPath}${testFileName.slice(1)}`
                                        : testFileName
                                    ...
                                    filePath &&
                                        dispatch(
                                            newTestFile({
                                                fileName: filePath,
                                                testFileName: absoluteTestFileName,
                                            })
                                        )
                                    handleCloseModal()
                                }}
                        ...
                        <div className="flex-none">
                            <span
                                className="close ravionai-pointer"
                ...
                <ul className="absolute bg-white border border-gray-300 rounded-md overflow-y-scroll max-h-48">
                    {filteredOptions.map((option, index) => (
                        <li
                            id={`option-${index}`}
                            key={option}
                            className={`px-4 py-2 ravionai-pointer hover:bg-gray-200 ${
