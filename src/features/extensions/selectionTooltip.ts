...
const ravionaiTooltipField = StateField.define<readonly Tooltip[]>({
    create: getravionaiTooltips,

    update(tooltips, tr) {
        // if the viewport changed, update the tooltips
        if (!tr.docChanged && !tr.selection) return tooltips
        // I think this is REALLY inefficient
        return getravionaiTooltips(tr.state)
    },

    provide: f => showTooltip.computeN([f], state => state.field(f)),
})

function getravionaiTooltips(state: EditorState): readonly Tooltip[] {
        ...
        .map(range => {
            return {
                ...
                create: (view: EditorView) => {
                    ...
                    dom.className = 'cm-tooltip-ravionai'
                    function btn(
                        titleText: string,
                        cmdText: string,
                        callback: any
...
const ravionaiTooltipBaseTheme = EditorView.baseTheme({
    '.cm-tooltip.cm-tooltip-ravionai': {
...
export function ravionaiTooltip() {
    return [ravionaiTooltipField, ravionaiTooltipBaseTheme]
}
