...
export interface Continueravionai {
    pos: number
}
export const insertravionaiEffect = StateEffect.define<Continueravionai>({
    map: (val, mapping) => ({ pos: mapping.mapPos(val.pos) }),
})
export const insertravionaiState = StateField.define<Continueravionai | null>({
    create: () => null,
    update(value, tr) {
        for (const effect of tr.effects) {
            if (effect.is(insertravionaiEffect)) {
                value = effect.value
            }
        }
        return value
