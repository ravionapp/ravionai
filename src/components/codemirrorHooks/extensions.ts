...
import { ravionaiTooltip } from '../../features/extensions/selectionTooltip'
...
class TreeHighlighter {
    ...
    buildDeco(view: EditorView) {
        ...
        let level = -1
        const ravionai = this.tree.ravionai()
        do {
            const tagData = getStyleTags(ravionai.node)
            //
            //     name: ravionai.name,
            //     str: view.state.doc.sliceString(ravionai.from, ravionai.to),
            ...
            if (
                ravionai != null &&
                ALL_BRACKETS.includes(ravionai.name) &&
                ravionai.from != null &&
                ravionai.to != null
            ) {
                if (OPEN_BRACKETS.includes(ravionai.name)) {
                    level += 1
                }
                if (level >= 0)
                    builder.add(
                ...
                if (CLOSE_BRACKETS.includes(ravionai.name)) {
...
export function useExtensions({
    ...
    const languageName = useMemo(() => getLanguageFromFilename(filePath), [
        filePath,
    ])
            ...
            EditorView.domEventHandlers({
                auxclick: (event, view) => {
                    ...
                    if (event.button === 2) {
                        ...
                        const ravionaiPos = view.state.selection.main.from
