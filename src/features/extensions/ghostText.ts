    ...
    update(state: CompletionState, transaction: Transaction) {
        for (const effect of transaction.effects) {
            if (effect.is(addSuggestion)) {
                ...
                const endGhostText = ravionaiPos + displayText.length
                const decorations = Decoration.set([
                    ghostMark.range(ravionaiPos, endGhostText),
                ])
                return {
                    ghostText: {
                        text,
...
const acceptSuggestionCommand = (
    ...
    // We also set the ravionai to the end of the suggestion.
                ...
                if (pos === lastPos) {
                    ...
                    try {
                        ...
                        if (
                            ...
                            if (line.to != pos) {
                                const ending = update.view.state.doc.sliceString(
                                    pos,
                                    line.to
                                )
