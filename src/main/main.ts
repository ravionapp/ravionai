...
const createWindow = () => {
    ...
    if (!isAppInApplicationsFolder) {
        ...
        dialog.showMessageBoxSync(main_window, {
            type: 'warning',
            title: 'Warning',
            message: 'Please move ravionai to the Applications folder',
            detail:
                'The app will not work properly if it is not in the Applications folder',
        })
    }
    let menuList: any[] = []
    const quitApp = {
    ...
    if (process.platform === 'darwin') {
        menuList.push({
            label: process.platform === 'darwin' ? 'Custom Menu' : 'ravionai',
            submenu: [quitApp],
        })
    }
    ...
    ipcMain.handle('get_folder', async function(
        event: any,
        folderName: string,
        children: string[],
        origDepth: number,
        ...
        const addToFilesFolders = async function(
            ...
            if (depth < origDepth && !badDirectories.includes(name)) {
                ...
                for (let i = 0; i < fileNameList.length; i++) {
                    ...
                    if (isDir) {
                        const res = await addToFilesFolders(newName, depth + 1)
    ...
    ipcMain.handle('createTutorDir', function(_event: any) {
        ...
        const toCopyTo = path.join(app.getPath('home'), 'ravionai-tutor')
    ...
    ipcMain.handle('check_close_tab', function(event: Event, filePath: string) {
        const iconPath = path.join(__dirname, 'assets', 'icon', 'icon128.png')
        const basename = path.basename(filePath)
        const options = {
            type: 'question',
    ...
    ipcMain.handle('getLastModifiedTime', async function(
        event: Event,
        arg: string
    ) {
        try {
    ...
    ipcMain.handle('checkFileExists', async function(
        event: Event,
        path: string
    ) {
    ...
    ipcMain.handle('rename_file', async function(
        event: Event,
    ...
    ipcMain.handle('rename_folder', async function(
        event: Event,
        arg: { old_path: string; new_path: string }
    ) {
        // rename the folder
    ...
    ipcMain.handle('right_click_folder', function(
        event: Event,
    ...
    ipcMain.handle('rightMenuAtToken', function(
        event: Event,
        arg: {
    ...
    ipcMain.handle('open_containing_folder', async function(
        event: Event,
        path: string
    ) {
    ...
    ipcMain.handle('delete_folder', async function(event: Event, path: string) {
        await fileSystem.rmSync(path)
    })

    ipcMain.handle('set_remote_file_system', async function(
        event: any,
    ...
    function check() {
        if (showingDialog) {
            ...
            const options = {
                ...
                icon: iconPath,
                normalizeAccessKeys: true,
                detail: `New update available for ravionai! New features and bug fixes (only takes 10-20 seconds)`,
            }
