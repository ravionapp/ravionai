...
import {
    editBoundaryEffect,
    insertravionaiEffect,
} from '../extensions/hackDiff'
...
const getBearerTokenHeader = (getState: () => unknown) => {
    const accessToken = (getState() as FullState).toolState.ravionaiLogin
        .accessToken
    if (accessToken) {
        return {
...
export async function getPayload({
    ...
    const userRequest = {
        ...
        currentFileContents,
        // Context surrounding the ravionai position
...
export const streamResponse = createAsyncThunk(
    ...
    async (
        ...
        try {
            const getFullState = () => getState() as FullState
            const conversationId = getFullState().chatState
            ...
            const processResponse = async () => {
                ...
                if (value.trim() == 'location') {
                    ...
                    const transactionFunction: CustomTransaction[] = generationJson.map(
                        change => ({
                            type: 'insert',
                            from: {
...
export const diffResponse = createAsyncThunk(
    ...
    async (type: 'lsp' | 'chat' | null, { getState, dispatch }) => {
        try {
            ...
            // If the ravionai is in a current pos, set the active line to the top of that
        ...
        switch (keypress) {
            ...
            case 'k':
                ...
                if (editorView) {
                    ...
                    const ravionaiPos = selection.from

                    //const preceedingPos = getPrecedingLinesPos(view, 20);
                    editorView.dispatch({
                        effects: insertravionaiEffect.of({
            ...
            case 'singleLSP':
                if (editorView) {
                    const currentErrorField = editorView.state.field(
                        activeLintField
                    )
