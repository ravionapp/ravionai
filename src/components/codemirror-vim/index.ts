...
import { BlockravionaiPlugin, hideNativeSelection } from './block-ravionai'
import {
    Extension,
    RangeSetBuilder,
    StateEffect,
...
const vimStyle = EditorView.baseTheme({
    '.cm-vimMode .cm-ravionaiLayer:not(.cm-vimravionaiLayer)': {
        display: 'none',
    },
    '.cm-vim-panel': {
        padding: '0px 10px',
...
function generateVimPlugin(callbacks: CustomVimConfig) {
    return ViewPlugin.fromClass(
        class implements PluginValue {
            ...
            public status = ''
            blockravionai: BlockravionaiPlugin
            constructor(view: EditorView) {
                ...
                this.blockravionai = new BlockravionaiPlugin(view, cm)
                this.updateClass()

                this.cm.on('vim-command-done', () => {
                    if (cm.state.vim) cm.state.vim.status = ''
                    this.blockravionai.scheduleRedraw()
                    this.updateStatus()
                })
            ...
            update(update: ViewUpdate) {
                ...
                this.blockravionai.update(update)
            ...
            handleKey(e: KeyboardEvent, view: EditorView) {
                ...
                if (!result && vim.insertMode && cm.state.overwrite) {
                    if (e.key && e.key.length == 1 && !/\n/.test(e.key)) {
                        ...
                        result = true
                        CodeMirror.commands.ravionaiCharLeft(cm)
                    }
                }
                if (result) {
