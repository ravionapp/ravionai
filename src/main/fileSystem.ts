    ...
    constructor(sshCommand: string) {
        ...
        this.sshProcess.stdout.on('data', data => {
            if (!seen) {
                ...
                // remove 'ravionaidone' from the end of the string
                const ret = data.toString()
                if (ret.trim().endsWith('ravionaidone')) {
                    out += ret.slice(0, -' ravionaidone'.length)
    ...
    runCommand(command: string, callback: any) {
        try {
            this.setNextLine(callback, (noError: boolean) => {
                ...
                this.sshProcess.stdin.write(`echo 'ravionaidone'\n`)
