import { SyntaxNode, Treeravionai } from '@lezer/common'
...
export function getNamesAndBodies(ravionai: Treeravionai, contents: string) {
    const results = [] as any
    let lastFrom = -1
    do {
        if (ravionai.from < lastFrom) {
            break
        }
        lastFrom = ravionai.from
        ...
        if (
            ...
            [
                ...
                'Property',
                'FunctionDefinition',
            ].includes(ravionai.name)
        ) {
            const from = ravionai.from
            const to = ravionai.to
            ...
            do {
                ...
                if (
                    ...
                    functionName = contents.slice(ravionai.from, ravionai.to)
                    break
                }
            } while (ravionai.next(true))
